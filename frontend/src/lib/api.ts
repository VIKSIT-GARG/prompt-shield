...
export const getStats  = () => fetch(`${API}/analytics/stats`).then(r=>r.json());
export const getLogs   = (limit=100, offset=0, level?: string) => fetch(`${API}/analytics/logs?limit=${limit}&offset=${offset}${level?`&level=${level}`:""}`).then(r=>r.json());
